...
const StudyNoteItem = (props: any) => {
        const { studyMember, selectWeek, onSelectMemberNote, selectedMemberNote, memberNoteContents, setMemberNoteContents } = props;
        ...
        const fetchMemberNoteContents = async () => {
            try {
            // 멤버의 공부노트 데이터를 받아옴
            const response = await fetch(`/api/study/note/${studyMember?.member?._id}?week=${selectWeek}`);
        ...
        <div onClick={handleClick}>
            ...
            <span style={{ color: "#748ffc" }}>{studyMember?.member?.name}</span>
            의 공부 노트
            </p>
            {studyMember?.member?._id === selectedMemberNote && (
            <div><div dangerouslySetInnerHTML={{ __html: memberNoteContents }} /></div>
            )}
        </div>
        );
    };
